(* Bohdan Purtell University of Florida
   ...
   IPv4 (OSI layer 3) transmit header generator. Sits *between* a layer-4 protocol
   (UDP/TCP) and the MII MAC:

   L4 (Udp_tx/Tcp_tx) ─(payload byte stream + metadata)→ Ipv4_tx ─→ MAC.s_axis

   This is the shared L3 block lifted out of the previously-fused udp_tx: it prepends the
   20-byte IPv4 header (with a correct header checksum) to whatever byte stream layer 4
   hands down, then streams that payload through unchanged. The MAC still wraps the result
   with the Ethernet header + FCS.

   Design decisions (carried over from the fused udp_tx):

   - ONE monotonic byte counter over the fixed 20-byte IPv4 header, decoded by offset into
     a 20-entry byte mux (a small ROM). Field boundaries live in the mux, not the FSM, so
     the FSM is just Idle -> Header -> Payload.

   - The IPv4 header checksum is mandatory and lives *before* the addresses it covers, so
     it can't be produced streaming. It is computed combinationally from the (latched)
     [l4_length] + constant fields (src/dst IP from [Config], protocol from the runtime
     input). Only [total_length] and [protocol] are dynamic.

   - Framing is single-source-of-truth: layer 4 asserts [l4_tlast] on the final byte of
...
module Make (C : Config) = struct
  ...
  let create (scope : Scope.t) (i : _ I.t) : _ O.t =
    ...
    (* IP total length = 20 + L4 length; total_length and protocol are the only dynamic
       inputs to the checksum. *)
    let l4_length = r.len_latch.value in
    let total_length = l4_length +:. ip_hdr_len -- "ip_total_len" in
    let protocol = i.I.protocol in
    let ttl_proto = const8 0x40 @: protocol in
    (* TTL=64 :: protocol *)

    (* ── IPv4 header checksum: one's-complement 16-bit sum of the header words (checksum
       field taken as 0), end-around carry, then complemented. ── *)
    ...
    in
    (* collapse bits above 16 back into the low 16 (end-around carry); applying twice is
       sufficient for a 20-bit input *)
    let fold_carry s =
    ...
    in
    let ip_checksum =
      ~:(select (fold_carry (fold_carry raw_sum)) ~high:15 ~low:0) -- "ip_checksum"
    in
    ...
    let header_bytes =
      ...
      ; lo16 total_length
      ; const8 0x00
      ; const8 0x00 (* identification *)
      ; const8 0x40
      ; const8 0x00 (* flags/frag: DF *)
      ; const8 0x40
      ...
      ; sm.switch
          ...
          ; ( Header
            , [ w.tvalid <--. 1 (* header byte always valid *)
              ; when_
                  mac_tready (* advance only when accepted *)
                  [ if_
                      (r.hdr_counter.value ==:. ip_hdr_len - 1)
          ...
          ; ( Payload
            , [ w.l4_ready <-- mac_tready (* forward backpressure to L4 *)
              ...
              ; when_
                  (i.I.l4_tvalid &: mac_tready &: i.I.l4_tlast)
