(* Bohdan Purtell University of Florida
   ...
   - [rx_tfirst] : 1-transfer SOF pulse on the first payload byte of a frame, which — for
     an IPv4 frame — is IPv4 header byte 0.
   - [rx_eth_type]: the latched Ethernet type, stable per frame. Used to filter: only
     0x0800 (IPv4) frames are parsed; everything else is flushed.

   What it does, mirroring Ipv4_tx in reverse:

   - Parses the fixed 20-byte IPv4 header (IHL=5 only for now — options / IHL>5 are
     dropped; see TODO). One monotonic byte counter walks the header; fields are latched
     by offset.
   - Verifies the IPv4 header checksum incrementally (one's-complement 16-bit sum over the
     20 header bytes, end-around carry; a correct header folds to 0xFFFF).
   - Strips the header and re-emits the L4 payload byte stream unchanged, with the IP
     payload length driving [m_tlast] (so the MAC's Ethernet zero-padding on short frames
...
module type Config = sig
  (* If true, a frame whose header checksum fails to verify is dropped (payload never
     reaches L4). If false, the payload is still forwarded and [checksum_ok] simply
     reports the result — useful for bring-up / lossy-link debugging. *)
...
module Make (C : Config) = struct
  ...
  module O = struct
    type 'a t =
      ...
      ; busy : 'a
      ; (* frame-level late-status channel (decoupled from the payload tlast). The FCS/CRC
           verdict for a frame only becomes known at the MAC's real end-of-frame — which,
           with MAC zero-padding, lands AFTER this block's m_tlast. So it cannot ride the
           payload stream; it is delivered here as a 1-cycle [frame_done] pulse (the MAC's
           rx_tlast, in any state) carrying the [frame_error] verdict, for a consumer to
           latch. *)
  ...
  let create (scope : Scope.t) (i : _ I.t) : _ O.t =
    ...
    (* one's-complement running sum: even header offsets carry the high byte of a 16-bit
       word, odd offsets the low byte. byte 0 is always the high byte, so its increment is
       computed directly (no reliance on the counter). *)
    let hi_inc = uresize (byte @: const8 0) ~width:20 in
    (* byte << 8 *)
    let lo_inc = uresize byte ~width:20 in
    let word_inc = mux2 (lsb idx) lo_inc hi_inc in
    (* idx odd -> low byte *)
    let next_sum = r.csum_acc.value +: word_inc in
    (* fold the end-around carry (>=16 bits) back into the low 16; twice is sufficient for
       a 20-bit accumulator. A valid header folds to 0xFFFF. *)
    let fold_carry s =
    ...
    in
    let is_ipv4_hdr0 =
      i.I.rx_eth_type ==:. ipv4_ethertype &: (byte ==:. 0x45)
      (* version 4, IHL 5 *)
    in
          ...
          [ ( Idle
            , [ w.m_ready <--. 1 (* ready to accept the SOF byte *)
              ; when_
                  ...
                  ; r.version_ihl <-- byte
                  ; r.hdr_counter <--. 1
                  ; if_ is_ipv4_hdr0 [ sm.set_next Header ] [ sm.set_next Flush ]
                    (* not IPv4 / options: drop *)
                  ; when_
    ...
    let keep =
      ...
      then
        reduce
          ~f:( |: )
          (bits_lsb r.src_ip.value
           @ bits_lsb r.dst_ip.value
           @ bits_lsb r.total_len.value
