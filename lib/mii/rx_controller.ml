(* Bohdan Purtell University of Florida
   ...
   controller. (Full unification with Common_types.States is deferred for that reason; see
   rx_controller_intf.ml.)
*)
...
(* Registered controls. The datapath reg-enables are pipelined one cycle behind the state
   (a Moore output) so they line up with the byte assembler's one-cycle-late byte_valid.
   mac_byte_count paces the multi-byte header states. *)
...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  (* helper functions *)
  let byte_counter_is (n : int) = r.mac_byte_count.value ==:. n in
  ...
  let fcs_present =
    rising_edge_delayed rising_edge ~n_cycles:1 i.rx_dv -- "fcs_present"
  in
  compile
    ...
    ; (* next-state logic, only on a valid assembled byte *)
      when_
        valid
        [ sm.switch
            ~default:[ sm.set_next IDLE ]
            [ IDLE, [ if_ stable [ sm.set_next PREAMBLE ] [ sm.set_next IDLE ] ]
            ; ( PREAMBLE
              , [ if_
                    stable
                    [ Always.switch
            ...
            ; ( DST_MAC
              , [ if_
                    ...
                    [ (* if_ (r.mac_byte_count.value ==:. 5) [ *)
                      if_
                        (byte_counter_is 5)
                        [ r.mac_byte_count <--. 0; sm.set_next SRC_MAC ]
            ...
            ; ( SRC_MAC
              , [ if_
                    stable
                    [ if_
                        (r.mac_byte_count.value ==:. 5)
                        [ r.mac_byte_count <--. 0; sm.set_next ETH_TYPE ]
                        [ r.mac_byte_count <-- r.mac_byte_count.value +:. 1
                        ; sm.set_next SRC_MAC
                        ]
                    ]
            ...
            ; ( ETH_TYPE
              , [ if_
                    stable
                    [ if_
                        (r.mac_byte_count.value ==:. 1)
                        [ r.mac_byte_count <--. 0; sm.set_next PAYLOAD ]
                        [ r.mac_byte_count <-- r.mac_byte_count.value +:. 1
  ...
  in
  { O.byte_assembler_en = i.en &: i.rx_dv
