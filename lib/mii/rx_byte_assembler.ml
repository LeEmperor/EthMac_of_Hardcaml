...
(* nibble_lo = first nibble seen (low half of the byte); nibble_hi = second. have_lo
   tracks whether we are mid-pair. byte_valid strobes on completion. *)
...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  compile
    ...
    ; when_
        ...
        [ if_
            r.have_lo.value
            [ r.nibble_hi <-- i.rx_data; r.have_lo <--. 0; r.byte_valid <--. 1 ]
            [ r.nibble_lo <-- i.rx_data; r.have_lo <--. 1 ]
        ]
    ];
