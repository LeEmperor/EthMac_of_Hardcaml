(* Bohdan Purtell University of Florida
   ...
   UDP (OSI layer 4) transmit header generator. Emits the UDP datagram byte stream —
   8-byte UDP header ++ application data — DOWN to the IPv4 layer (Ipv4_tx), which
   prepends the IP header; the MAC then wraps with Ethernet.

   app data ─→ Udp_tx ─(UDP datagram bytes + metadata)→ Ipv4_tx ─→ MAC

   This used to be a fused IPv4+UDP generator (one 28-byte header mux). The IPv4 half was
   lifted into Ipv4_tx; what remains here is just the 8-byte UDP header and the payload
   passthrough, plus the small metadata IPv4 needs to build its header ([l4_length],
   [protocol]) and the start pulse it forwards.

   - ONE monotonic byte counter over the fixed 8-byte UDP header, decoded by offset into
     an 8-entry byte mux. FSM is Idle -> Header -> Payload.

   - Framing: Udp_tx owns the datagram's [m_tlast] — it asserts tlast on its final byte
     (last app-data byte, or the 8th header byte for a zero-length datagram). Ipv4_tx
     forwards that tlast to the MAC.

   - UDP checksum over IPv4 is optional and emitted as 0x0000 for now. A real checksum
     needs the IPv4 pseudo-header (src/dst IP + protocol + udp_length); when implemented,
...
module Make (C : Config) = struct
  ...
  let create (scope : Scope.t) (i : _ I.t) : _ O.t =
    ...
    let header_bytes =
      [ const8 (C.src_port lsr 8)
      ; const8 (C.src_port land 0xff)
      ; const8 (C.dst_port lsr 8)
      ; const8 (C.dst_port land 0xff)
      ; hi16 udp_length
      ; lo16 udp_length
      ; const8 0x00
      ; const8 0x00 (* UDP checksum = 0 (optional over IPv4) *)
      ]
    in
      ...
      ; sm.switch
          ...
          ; ( Header
            , [ w.tvalid <--. 1 (* header byte always valid *)
              ; when_
                  (r.hdr_counter.value ==:. udp_hdr_len - 1 &: (r.payload_rem.value ==:. 0)
                  )
                  [ w.tlast <--. 1 ]
                (* describe the final beat even while stalled *)
              ; when_
                  l4_tready (* advance only when accepted *)
                  [ if_
                      (r.hdr_counter.value ==:. udp_hdr_len - 1)
                      [ r.hdr_counter <--. 0
                      ; if_
                          (r.payload_rem.value ==:. 0) (* zero-length datagram *)
          ...
          ; ( Payload
            , [ w.p_ready <-- l4_tready (* forward backpressure *)
              ...
              ; when_
                  (payload_tvalid &: l4_tready)
                  [ if_
                      (r.payload_rem.value ==:. 1)
                      [ r.payload_rem <--. 0; r.busy <--. 0; sm.set_next Idle ]
  ...
  let hierarchical ?instance scope i =
    ...
    H.hierarchical ?instance ~scope ~name:"udp_tx" create i
  ;;
end
