(* Bohdan Purtell University of Florida
   ...
   Ipv4_rx has already stripped the Ethernet + IPv4 headers and hands up the
   UDP *datagram* (8-byte UDP header ++ application data) as an AXI-Stream, plus the L4
   metadata it parsed out of the IP header:

   - [rx_tfirst] : 1-transfer SOF pulse on the first datagram byte (= UDP header byte 0).
   - [ip_protocol] : IP protocol number, stable per frame. Only 17 (UDP) is parsed here;
     anything else is flushed (a UDP block must never emit TCP/ICMP payload).
   - [ip_src_ip]/[ip_dst_ip] : peer/local addresses, stable per frame. Passed straight
     through so the application learns the sender.

   What it does, mirroring Udp_tx in reverse:

   - Parses the fixed 8-byte UDP header. One monotonic byte counter walks the header;
     fields (src_port, dst_port, length, checksum) are latched by offset, big-endian, via
     [accum] — the same shift-in helper Ipv4_rx uses.
   - Strips the header and re-emits the application payload byte stream unchanged, with
     the UDP length field driving [m_tlast] (independent of the IP-level length, so a
     short UDP length inside a larger IP payload is still honoured).
   - Surfaces the metadata the application needs: [src_port]/[dst_port],
     [udp_length]/[payload_length], [src_ip]/[dst_ip], plus an [app_start] SOF pulse — the
     RX mirror of the [{start, payload_len}] contract Udp_tx consumes.

   Port policy (Config):
   - [drop_on_port_mismatch = false] (default): accept every datagram, forward its
     payload, and simply *report* src/dst port. [port_match] is still computed against
     [expected_dst_port] as an informational flag.
   - [drop_on_port_mismatch = true]: behave like a bound socket — datagrams whose dst_port
     /= [expected_dst_port] are dropped in Flush and never reach the app.
   ...
   udp_length — all available from Ipv4_rx) plus the whole datagram; TX currently emits
...
module type Config = sig
  (* If true, datagrams whose dst_port /= [expected_dst_port] are dropped (payload never
     reaches the application). If false, every datagram is forwarded and [port_match] just
     reports the comparison — useful for bring-up. *)
  val drop_on_port_mismatch : bool
  val expected_dst_port : int

  (* keep-folding of internal debug signals into [keep] so synthesis can't prune them from
...
module Make (C : Config) = struct
  ...
  module O = struct
    type 'a t =
      ...
      ; crc_error : 'a
          (* IPv4/MAC reported a bad frame (rx_tuser) — note: this legacy tlast-aligned
  ...
  let create (scope : Scope.t) (i : _ I.t) : _ O.t =
    ...
    (* shift a byte into a wider big-endian field register (lifted from udp.ml / shared
       with ipv4_rx). *)
      ...
      ; sm.switch
          ...
          [ ( Idle
            , [ w.m_ready <--. 1 (* ready to accept the SOF byte *)
              ; when_
                  (i.I.rx_tvalid &: i.I.rx_tfirst)
                  [ r.busy <--. 1
                  ; r.crc_err <--. 0
                  ; r.first_pend <--. 0
                  ; accum r.src_port byte (* header byte 0 = src_port high *)
                  ; r.hdr_counter <--. 1
                  ; if_ is_udp [ sm.set_next Header ] [ sm.set_next Flush ]
                    (* not UDP: drop the datagram *)
                  ; when_
                      i.I.rx_tlast (* 1-byte runt frame: abort *)
          ...
          ; ( Flush
            , [ w.m_ready <--. 1 (* swallow, emit nothing *)
              ; when_ (i.I.rx_tvalid &: i.I.rx_tlast) [ r.busy <--. 0; sm.set_next Idle ]
              ] )
          ]
      ];
    (* TODO(udp-rx checksum): verify the UDP checksum. Build the IPv4 pseudo-header from
       [{ip_src_ip, ip_dst_ip, 0x0011 (proto), udp_length}] and one's-complement sum it
       together with the whole datagram (streamed in Payload); a valid datagram folds to
       0xFFFF, and a received checksum field of 0x0000 means the sender disabled it (skip
       the check, report ok). For now checksum_ok is a stub so downstream can be wired up;
       TX emits 0x0000 today. *)
    ...
    let keep =
      ...
      then
        reduce
          ~f:( |: )
          (bits_lsb r.src_port.value
           @ bits_lsb r.dst_port.value
           @ bits_lsb r.length.value
           @ bits_lsb r.checksum.value
  ...
  let hierarchical ?instance scope i =
    ...
    H.hierarchical ?instance ~scope ~name:"udp_rx" create i
  ;;
end
