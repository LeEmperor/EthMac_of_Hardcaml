...
(* Module: "rx_crc_legacy_assertion_test.ml" *)

(* Legacy Assertion Test: Rx_crc

   Deprecated standalone simulation and manual assertion harness, formerly
   [test/mii/crc_tb.ml]. The unit, Quickcheck, and expect suites in this directory
   supersede it.
...
let () =
  ...
  let run sim =
    ...
    let cycle () =
      ...
      then
        printf
          "  data=0x%02x valid=%d  hw_crc=0x%08x  crc_valid=%d\n"
    ...
    reset ();
    (* List.iter is pretty cool, almost like that one Rust function that does a function
       on each thing in an iterable *)
