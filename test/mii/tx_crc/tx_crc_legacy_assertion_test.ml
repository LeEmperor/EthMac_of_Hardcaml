...
(* Module: "tx_crc_legacy_assertion_test.ml" *)

(* Legacy Assertion Test: Tx_crc

   Deprecated standalone simulation and manual assertion harness, formerly
   [test/mii/tx_crc_tb.ml]. The unit, Quickcheck, and expect suites in this directory
   supersede it.
...
let () = print_endline "=== Running MAC TX CRC Testbench ==="
...
let () =
  ...
  let run sim =
    ...
    let cycle () =
      ...
      then
        printf
          "  data=0x%02x valid=%d  crc_reg=0x%08x  fcs_byte(sel=%d)=0x%02x\n"
    ...
    printf
      "  hw crc_out = 0x%08x  hw FCS = 0x%08x  %s\n"
      hw_raw
      hw_fcs
      (if hw_fcs = expected_fcs then "PASS" else "FAIL");
    ...
    List.iteri [ 0x26; 0x39; 0xF4; 0xCB ] ~f:(fun sel expected ->
      ...
      printf
        "  byte_sel=%d: fcs_byte=0x%02x  (expect 0x%02x)  %s\n"
        sel
        got
        expected
    ...
    t_en <-- 0;
    idle ();
    (* en=0 resets accumulator back to 0xFFFFFFFF *)
    t_en <-- 1;
    List.iter data_bytes ~f:feed_byte;
    idle ();
    let hw_fcs2 = Bits.to_int_trunc !(outputs.crc_out) lxor 0xFFFFFFFF in
    printf
      "  hw FCS after reset+rerun = 0x%08x  (expect 0x%08x)  %s\n"
      hw_fcs2
      expected_fcs
      (if hw_fcs2 = expected_fcs then "PASS" else "FAIL");
    ...
    printf
      "  sw FCS = 0x%08x  hw FCS = 0x%08x  %s\n"
      sw_fcs3
      hw_fcs3
      (if hw_fcs3 = sw_fcs3 then "PASS" else "FAIL");
  ...
  in
  if waveform_enabled
  then Out_channel.with_file "waves_tx_crc.vcd" ~f:(fun oc -> run (Vcd.wrap oc sim))
  else run sim
;;
