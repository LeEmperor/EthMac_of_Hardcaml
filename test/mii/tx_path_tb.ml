...
let create_sim () =
  ...
  let sim =
    Sim.create
      ...
      (Mac_top.create ~rx_fifo_for_sim:true scope)
  in
...
(* Read all nibbles that come out of tx_d while tx_en is 1 Waits up to [max_wait] cycles
   for tx_en to first assert, then collects until it deasserts

   Returns the reassembled byte list (lo nibble first per byte)
*)

let collect_frame ~label ~cycle ~outputs ~max_wait =
  let tx_en () = Bits.to_bool !(outputs.Mac_top.O.tx_en) in
  (* ! grabs value from a container ref *)
  ...
  if !waiting then failwithf "%s: tx_en did not assert within %d cycles" label max_wait ();
  (* collect while tx_en is high, bounded so an underflow (FSM stall) fails loudly instead
     of hanging the sim forever *)
  let guard = ref 0 in
...
let () =
  ...
  let run sim =
    ...
    let reset () =
      ...
      inputs.en <-- 1
    in
    (* hardcoded frame header constants — must match tx_datapath.ml: dst = broadcast, src
       = locally-administered (02:..), ethertype 0x9999 *)
    ...
    (* ── parametrized frame test ── Drives [data] (the application payload) into the TX
       FIFO with tlast on the final byte, then checks the reassembled MII frame. The
       controller is expected to:
       - transmit exactly [data] when |data| >= 46, or
       - zero-pad up to 46 bytes when |data| < 46, and to append an FCS computed over
         dst/src/ethertype + the (padded) payload. Returns true on full PASS. *)
    let run_frame_test ~label ~data =
      ...
      let check_region name offset expected =
        ...
        let got =
          ...
          then List.sub frame ~pos:offset ~len:n
            ...
            List.sub
              frame
              ~pos:(min offset (List.length frame))
      ...
      then
        printf
          "frame length: FAIL (got %d, expect %d)\n"
          (List.length frame)
          exp_frame_len;
      ...
      ok
    in
    (* sequential lets (not a list literal) so tests run top-to-bottom — OCaml leaves
       list-element evaluation order unspecified (right-to-left in practice) *)
    (* exactly the minimum — tlast-terminated, no padding (baseline) *)
    let r1 =
      run_frame_test ~label:"test 1: 46B min" ~data:(List.init 46 ~f:(fun i -> i + 1))
    in
    (* sub-minimum — heavy padding *)
    let r2 =
      run_frame_test ~label:"test 2: 18B pad" ~data:(List.init 18 ~f:(fun i -> 0x80 + i))
    in
    ...
    let r4 =
      run_frame_test ~label:"test 4: 45B pad" ~data:(List.init 45 ~f:(fun i -> i + 1))
    in
    (* one over the minimum — no padding, variable length above 46 *)
    let r5 =
      run_frame_test ~label:"test 5: 47B nopad" ~data:(List.init 47 ~f:(fun i -> i + 1))
    in
    (* comfortably above minimum *)
    let r6 =
      run_frame_test
        ~label:"test 6: 64B nopad"
        ~data:(List.init 64 ~f:(fun i -> i * 3 land 0xFF))
    in
  ...
  in
  if waveform_enabled
  then Out_channel.with_file "waves_tx_top.vcd" ~f:(fun oc -> run (Vcd.wrap oc sim))
  else run sim
;;
