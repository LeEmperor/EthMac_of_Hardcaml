...
(* Testbench Support: Rx_byte_assembler
   ...
   Tags: [{ "ACTIVE" ; "TEST" ; "TESTBENCH" ; "PERSONAL_REFERENCE" ; "COMMON_ITEMS" }]
*)
...
module Testbench = struct
  ...
  (* perhaps this might be abstractable for any module that has a reset signal, and given
     set of inputs that need to be 0 during a reset condition? *)
  let reset ?(num_cycles = 1) (handler : Step.Handler.t @ local) =
    Step.delay ~num_cycles handler (inputs ~reset:true ~en:false ~rx_data:0)
  ;;

  (* is this a partial application or just a tuple lmao *)
  ...
  let observe_byte ~after_low_nibble ~after_high_nibble =
    let valid_after_low_nibble = Bits.to_bool after_low_nibble.Dut.O.byte_valid in
    let valid_after_high_nibble = Bits.to_bool after_high_nibble.Dut.O.byte_valid in
  ...
  let run_bytes bytes =
    ...
    run_with_timeout ~timeout ~testbench:(fun handler initial_outputs ->
      scenario ~bytes handler initial_outputs)
  ;;

  (* i think of this like a uvm sequence - probably unhealthy *)
  (* these really sohuld be composed elsewhere but i guess i don't have enough things to
     jutsify such a thing - also for TLM items its probably best to put them here so that
     whoeve rrefers to this module can have the items exist in an agnostic state
     regardless of the backing simulation (ie cycle vs event) *)
    ...
    let testbench (handler : Step.Handler.t @ local) _initial_outputs =
      ...
      let while_disabled =
        List.map disabled_nibbles ~f:(fun nibble ->
          Step.cycle handler (inputs ~reset:false ~en:false ~rx_data:nibble)
          |> Step.O_data.after_edge)
      in
  ...
  (* this one instead works with the remaining things after we halt at some random point;
  ...
  let run_reset_mid_byte ~discarded_low ~byte =
    let testbench (handler : Step.Handler.t @ local) _initial_outputs = (* heeheheeheehaw warning 67 *)
      ...
      (* eventually gotta stop doing this parenthesis thing but really helps me to reason
         ...
         write it into formatter tooling myself lmao *)

      (* the thought is a structured binding can be used here, but the knowledge that
         execution order is not guaranteed for tuples in this language (thanks OCaml
