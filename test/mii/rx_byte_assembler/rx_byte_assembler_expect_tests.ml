...
(* Expect Test Suite: Rx_byte_assembler
   ...
   Golden-output tests presenting cycle-by-cycle behavior as readable traces, including
   disabled cycles and reset during a partially assembled byte.

   Tags: [{ "ACTIVE" ; "TEST" ; "EXPECT_TEST" ; "EXPECT" ; "PERSONAL_REFERENCE" }]
*)
...
(* God I hate these expect sexps: running the test and then pasting in the output is
   actually kinda goated; i think theres a tool that does that called like tuareg or
   something but it doesn't have a neovim port (i haven't looked); might have to write one
   myself *)
...
let%expect_test "disabled cycles do not consume nibbles" =
  ...
  (* introduction of snapshot concept that is essentially and unfinished Observation *)
  [%expect
    {|
