...
(* Executable: "rx_path_tb.ml"
   ...
   design notes: obviously as an executable this generates the waveforms that we might
   want

   but is it possible for us to have this exist as a test layer item? do test-layer items
   in the alcotest framework have to adhere to being executables in the first place? can
   tests have system-level sideeffects such as generated log files or other things like
   waveform files?
...
let () =
  ...
  let run sim =
    ...
    let collect_output () =
      ...
      let tfirst_at = ref [] in
      (* byte indices where m_axis_tfirst pulsed *)
      (* Sample BEFORE cycling: in cut-through FIFO the head word is already at the
         output. Read while tvalid&tready, then cycle to advance to next word. *)
      for i = 0 to 30 do
        ...
        then
          printf
            "  [%2d] valid=%d data=0x%02x last=%d user=%d first=%d\n"
            i
            (if v then 1 else 0)
            b
            (if l then 1 else 0)
    ...
    in
    (* Send a valid frame and assert the RX contract end-to-end: exact payload bytes,
       tlast present, tuser=0, SOF on the first byte only, latched ethertype (byteswapped
       — see note below). *)
    let run_valid ~label ~eth_type ~payload ~payload_len =
      printf
        ...
        eth_type;
      reset ();
      (* tready stays 0 (from reset) so the FIFO buffers the whole frame; then tready=1
         drains it and we observe tlast/tuser/tfirst *)
      send_frame_with_fcs
        ~eth_type
        ~payload
        ~payload_len
      ...
      (* This TB emits its 16-bit fields LSB-first (send_bytes), so eth_type 0x4521 hits
         the wire as [0x21,0x45] and the datapath — accumulating MSB-first — latches
         0x2145, the byteswap. (A real big-endian wire ethertype yields the correct
         numeric value.) *)
      ...
      printf
        ...
        (not user)
        sof_ok
        (Bits.to_int_trunc !(outputs.rx_eth_type))
        eth_ok;
      ok
    in
    (* ── test 1: valid frame (5B payload) ── *)
    let t1_ok =
      run_valid ~label:"test 1" ~eth_type:0x45_21 ~payload:0x12_34_56_78_90 ~payload_len:5
    in
    ...
    let t2_ok =
      run_valid
        ~label:"test 2"
        ~eth_type:0x08_00
        ~payload:0xA1_B2_C3_D4_E5_F6_07
        ~payload_len:7
    in
    (* ── test 3: bad FCS — expect tuser=1 on tlast ── *)
    printf "\n-- [test 3] bad FCS (all bytes inverted) --\n";
    let eth_type = 0x45_21
    and payload = 0x12_34_56_78_90
    and payload_len = 5 in
    reset ();
    let bad_fcs =
    ...
    printf
      "test 3: %s  (got_tlast=%b  tuser=1=%b)\n"
      (if t3_ok then "PASS" else "FAIL")
      (Option.is_some last3)
      user3;
