...
(* Legacy Assertion Test: Ipv4_tx
   ...
   Feeds a synthetic layer-4 byte stream into Ipv4_tx and checks that it prepends the
   correct 20-byte IPv4 header and passes the L4 payload through unchanged:

   [0..19] IPv4 header (version/IHL, total_length, TTL/proto, checksum, src/dst IP) [20..]
   the L4 payload, verbatim m_tlast forwarded from the L4 stream's l4_tlast on the final
   byte

   The golden header checksum is recomputed in OCaml (one's-complement sum, end-around
   carry, complement) so an RTL checksum bug shows up as a mismatch on bytes [10..11].
   total_length = 20 + l4_length; protocol is the runtime input.

   Observation model: Ipv4_tx's stream outputs are Mealy, and Cyclesim.cycle leaves the
   output refs POST-edge. Header bytes and payload[0..n-2] are observed directly; the
   final payload byte + tlast collapse to Idle on the accepting edge and are reconstructed
   from completion (busy 1->0). tx_start is a one-cycle Idle pulse, unobservable
   post-edge, so it is counted via busy 0->1 rising edges (it is set in the same
   Idle&start branch). See [drive] for the full rationale.
   ...
   Not tested (by design): a 0-byte L4 payload. Framing is L4-driven — Ipv4_tx waits in
   Payload for l4_tvalid & l4_tlast to close the datagram, so a datagram with no payload
   byte to carry tlast would never complete. Every real L4 (UDP/TCP) always supplies at
   least its own header bytes.
...
let () =
  ...
  let reset () =
    ...
    i.reset <-- 0
  in
  (* Drive ONE datagram of L4 bytes [l4] (protocol [protocol]) through Ipv4_tx, assuming
     the FSM is currently Idle, and collect the emitted stream.

     Observation model (verified against RTL with a phase-trace): [Cyclesim.cycle] leaves
     the output refs in their POST-edge values. Header bytes and payload bytes [0 .. n-2]
     are observed directly on the m_tdata/m_tvalid handshake. The FINAL payload byte and
     its tlast are NOT observable: accepting it returns the FSM to Idle on the same edge,
     collapsing m_tvalid to 0. We detect completion via busy 1->0 and append the known
     last byte (l4[n-1]); its byte-perfect emission is verified end-to-end in
     udp_mac_top_tb.

     tx_start is likewise a one-cycle Mealy pulse in Idle and is unobservable post-edge;
     it is asserted in the SAME Idle&start branch that sets busy, so we count busy 0->1
     rising edges as the tx_start pulse count (must be exactly 1).
  ...
  let drive ~l4 ~protocol ~stall_every =
    ...
    (* pulse start on the first iteration only *)
    let tx_starts = ref 0 in
    (* busy 0->1 rising edges == tx_start pulses *)
    ...
    while (not !saw_last) && !guard < 500 do
      ...
      (* AXI source: present byte[ptr] and hold it valid until accepted. Keep the FINAL
         byte (with tlast) asserted even past ptr=n until the datagram actually completes
         — otherwise a 1-byte datagram, whose only byte is also its tlast beat, would have
         valid yanked before the FSM's completing Payload cycle and would hang. (For n>=2,
         ptr never reaches n before completion, so this only affects n=1.) *)
      ...
      (* completion: busy 1->0. The final payload byte usually collapses to Idle post-edge
         (n>=2) and must be reconstructed; but for n=1 it was already observed on the
         Payload-entry cycle. Reconstruct only if genuinely missing (observed payload
         bytes < n). *)
      if !prev_busy && not busy
      then (
        let observed_payload = List.length !collected - 20 (* IPv4 header bytes *) in
        if observed_payload < n then collected := !collected @ [ List.nth_exn l4 (n - 1) ];
    ...
    then
      failwithf
        "IPv4 TX timed out after %d cycles: l4_len=%d protocol=%d stall_every=%d \
         accepted=%d"
        !guard
        n
        protocol
  ...
  let verify ~label ~l4 ~protocol (got, tlast_idx, tx_starts) =
    ...
    if not len_ok
    then printf "  length: FAIL got %d expect %d\n" (List.length got) exp_len;
    ...
    then (
      ...
      printf "    got:      %s\n" (show got));
    let field off len =
      List.sub got ~pos:off ~len |> List.map ~f:(sprintf "%02x") |> String.concat ~sep:""
    in
    if bytes_ok
    then
      printf
        "  ip.total_len = 0x%s  ip.proto = 0x%s  ip.cksum = 0x%s\n"
        (field 2 2)
        (field 9 1)
        (field 10 2);
    let tlast_ok = tlast_idx = exp_len - 1 in
    if not tlast_ok
    then printf "  tlast index: FAIL got %d expect %d\n" tlast_idx (exp_len - 1);
  ...
  in
  (* Two datagrams through one FSM instance, no reset between them: exercises re-arming
     from Idle after a completed datagram (tx_start must fire again, header/checksum must
     be recomputed for the new length/protocol). *)
  let check_back_to_back () =
    printf "\n-- [test 8: back-to-back A(12B UDP) then B(30B TCP)] --\n";
    reset ();
    let la = make_payload 12
    and lb = make_payload 30 in
  ...
  let results =
    ...
    ; check ~label:"test 5: 1B UDP" ~l4_len:1 ~protocol:17 ~stall_every:0
      (* aggressive stall: mac_tready low every other cycle — bubbles fall on header bytes
         AND the tlast cycle *)
