(* Bohdan Purtell University of Florida
   ...
   Drives a synthetic Ethernet-payload byte stream — exactly what Mac_top.m_axis presents
   (rx_tdata/tvalid/tlast/tuser + the rx_tfirst SOF pulse and latched rx_eth_type) — and
   checks that Ipv4_rx:

   - strips the 20-byte IPv4 header and re-emits the L4 payload verbatim,
   - drives m_tlast off the IP total_length (so trailing Ethernet zero-padding on short
     frames is dropped, not leaked to L4),
   - surfaces the right metadata (protocol, payload_length, src/dst IP),
   - verifies the header checksum (checksum_ok), and honours the drop_on_bad_checksum
     policy,
...
let () =
  ...
  let reset () =
    ...
    i.reset <-- 0
  in
  (* Present [frame] (the full Ethernet payload: IP header ++ L4 ++ padding) to the DUT
     the way the MAC would, one byte per accepted cycle, with tfirst on byte 0 and
     tlast+tuser on the final byte. [stall_every]=k inserts an l4_tready=0 bubble every k
     transfers to exercise backpressure. Collects the stripped L4 stream the DUT emits,
     plus the metadata sampled at l4_start.

     Cyclesim phase note: this DUT's payload/SOF outputs are Mealy (combinational from
     state + inputs). After [Cyclesim.cycle], the output ports hold comb(next_state,
     current_input) — the valid/last/first *qualifier* belongs to the NEXT cycle's state
     while tdata belongs to the CURRENT input. So we read the qualifiers one iteration
     late: the qualifier for the byte on the bus during cycle N is the value we read at
     the END of iteration N-1 (= comb(S_N, ..)). We hold those in [q_*] and pair them with
     the byte we're presenting this iteration. tdata itself is just the input byte we
     drive, so we collect that directly rather than reading it back. *)
  let run ~label ~expect_output ~frame ~eth_type ~fcs_bad ~stall_every =
    ...
    (* (protocol, payload_length, src, dst) at l4_start *)
    ...
    while (!ptr < n || (expect_output && not !saw_ip_last)) && !guard < guard_limit do
      ...
      (* Transfer of the byte on the bus THIS cycle, qualified by the S_N reads: an L4
         byte moves when the DUT is emitting valid and we grant ready. *)
    ...
    then
      failwithf
        ...
        n
        ();
    if expect_output && not !saw_ip_last
    then
      failwithf
  ...
  let frame =
    ip_header ~l4_len:26 ~protocol:17 ~src_ip ~dst_ip ~corrupt_cksum:false @ l4
  in
  let got, tlast_idx, meta, csum_ok, crc_err =
    run
      ~label:"test 1: 26B UDP"
      ~expect_output:true
      ~frame
      ~eth_type:0x0800
      ~fcs_bad:false
  ...
  let got, tlast_idx, meta, csum_ok, _ =
    run
      ~label:"test 2: padded UDP"
      ~expect_output:true
      ~frame
      ~eth_type:0x0800
      ~fcs_bad:false
  ...
  let frame =
    ip_header ~l4_len:20 ~protocol:6 ~src_ip ~dst_ip ~corrupt_cksum:false @ l4
  in
  let got, tlast_idx, meta, csum_ok, _ =
    run
      ~label:"test 3: stalled TCP"
      ~expect_output:true
      ~frame
      ~eth_type:0x0800
      ~fcs_bad:false
  ...
  let frame =
    ip_header ~l4_len:16 ~protocol:17 ~src_ip ~dst_ip ~corrupt_cksum:true @ l4
  in
  let got, _, _, csum_ok, _ =
    run
      ~label:"test 4: bad checksum"
      ~expect_output:false
      ~frame
      ~eth_type:0x0800
      ~fcs_bad:false
  ...
  let frame =
    ip_header ~l4_len:24 ~protocol:17 ~src_ip ~dst_ip ~corrupt_cksum:false @ l4
  in
  let got, tlast_idx, _, csum_ok, crc_err =
    run
      ~label:"test 6: bad FCS"
      ~expect_output:true
      ~frame
      ~eth_type:0x0800
      ~fcs_bad:true
