...
let () =
  ...
  let reset () =
    ...
    i.reset <-- 0
  in
  (* Drive one datagram from Idle. Sampling the Before-edge output interface makes every
     scoreboard event correspond to the edge that actually accepts it; this avoids the
     post-edge phase ambiguity of the original smoke test. *)
  let drive
    ?(stall_every = 0)
    ?(bubble_every = 0)
    ...
    while (not !done_) && !guard < guard_limit do
      ...
      in
      let final_stall = targeting_final_beat && !final_stalls_used < final_stall_cycles in
      if final_stall then incr final_stalls_used;
      let periodic_stall = stall_every > 0 && !guard % (stall_every + 1) = stall_every in
      ...
      if Bits.to_int_trunc !(o_before.protocol) <> 17 then protocol_constant := false;
      ...
      (match !previous_stalled_beat with
       ...
       | None -> ());
      previous_stalled_beat := if tvalid && not ready then Some (tdata, tlast) else None;
      if tvalid && not ready then saw_downstream_stall := true;
      let expected_payload_ready = header_complete && n > 0 && ready in
      if Bool.(payload_ready <> expected_payload_ready) then payload_ready_ok := false;
      if tvalid && ready
      then (
  ...
  let expect_datagram label payload result =
    ...
    expect
      ...
      (List.equal Int.equal result.tlast_indices [ List.length expected - 1 ]);
    ...
    expect (label ^ ": output stable while stalled") result.stream_stable_while_stalled
  in
  let run_clean ?stall_every ?bubble_every ?final_stall_cycles ?mutate_len payload =
    ...
    drive
      ...
      ?bubble_every
      ?final_stall_cycles
      ?mutate_len_after_start:mutate_len
      payload
  in
