...
(* Module: "udp_rx_mac_top_legacy_assertion_test.ml" *)
(* Superseded UDP-over-MAC RX harness. Tags: [{ "DEPRECATED" ; "TEST" }] *)

(* Top-level UDP receive integration testbench.

   Drives real MII RX nibbles (preamble / SFD / dst_mac / src_mac / ethertype / IPv4
   header / UDP header / app payload / FCS) into [Udp_rx_mac_top] and checks the recovered
   application datagram + metadata out of the UDP layer:

   MII nibbles ─→ Mac_top ─→ Ipv4_rx ─→ Udp_rx ─→ app

   This is the RX mirror of udp_mac_top_tb.ml, and the piece that udp_ipv4_rx_tb.ml could
   not cover: the real MAC-RX → m_axis → Ipv4_rx handoff (SOF + rx_eth_type sidebands,
   cut-through RX FIFO), exercised from the wire.

   Single-clock Cyclesim with [rx_fifo_for_sim:true] (the async RX FIFO is swapped for a
   sync one, so rx_clock and tx_clock advance together).
*)
...
(* Full Ethernet *payload* (IPv4 header ++ UDP datagram), optionally MAC-padded to the
   46-byte minimum so the RX pad-strip path is exercised. *)
let ipv4_udp_eth_payload
...
(* Drive one whole Ethernet frame on the MII RX pins, then drain the recovered UDP
   application stream. app_tready is held low while the frame is clocked in so the MAC
   FIFO buffers it, then raised to drain — mirroring rx_path_tb. *)
let run ?(eth_type = [ 0x08; 0x00 ]) ?(corrupt_fcs = false) eth_payload =
  ...
  (* crc_error / checksum_ok are registered in Udp_rx (updated the cycle after its own
     rx_tlast), so they settle one cycle behind app_tlast. OR them across the whole drain
     window (after the frame starts) rather than sampling at tlast. *)
  for _ = 0 to 79 do
    if bit o.app_start
    then
      meta
    ...
    if bit o.app_tvalid then got := !got @ [ Bits.to_int_trunc !(o.app_tdata) ];
...
let () =
  ...
  in
  expect_datagram
    ~label:"test 1: nominal UDP datagram over MII"
    ~result:r1
    ~payload:payload1;
  ...
  in
  expect_datagram
    ~label:"test 2: short datagram, MAC padding stripped"
    ~result:r2
    ~payload:payload2;
  ...
  (* test 4: bad Ethernet FCS. The payload is still forwarded (FCS is a status sideband,
     not a gate), AND the bad-frame verdict now reaches the application via the
     frame-level late-status channel (Ipv4_rx/Udp_rx frame_done/frame_error,
     latched-and-held in Udp_rx_mac_top). This exercises the exact-fit (non-padded) path
     where the MAC's tlast+tuser coincide with the last L4 byte. *)
  let r4 =
    run
      ~corrupt_fcs:true
      (ipv4_udp_eth_payload
  ...
  check
    "payload still delivered on a bad-FCS frame"
    (List.equal Int.equal r4.payload payload1);
  check "app-layer crc_error asserted on bad FCS" r4.crc_error;
  (* test 5: bad Ethernet FCS on a MAC-padded frame — the padding pushes the FCS verdict
     PAST L4's tlast, the case a tlast-aligned flag cannot carry. The frame-level channel
     still delivers it. *)
  ...
  check
    "padded payload still delivered on a bad-FCS frame"
