...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  let ptr = Always.Variable.reg ~enable:vdd ~width:11 spec_tx in
  (* app-byte index *)
      ...
      [ sm.switch
          [ ( Tx_trigger_states.Idle
            , [ when_
                  ...
                  [ ptr <--. 0; started <--. 0; sm.set_next Tx_trigger_states.Stream ]
              ] )
          ...
          ; ( Tx_trigger_states.Busy
            , [ when_
                  (~:wire_udp_busy &: ~:wire_tx_busy)
                  [ sm.set_next Tx_trigger_states.Idle ]
              ] )
          ]
      ]);
  (* ── RX drain: pop one recovered UDP byte per second (from the RX harness). Gating
     app_tready backpressures the whole IPv4/UDP RX chain, so the datagram buffers in the
     MAC's async RX FIFO and its bytes walk out onto led[3:0]. *)
  ...
  let saw_valid_datagram =
    ...
    -- "saw_valid_datagram"
  in
  (* ── Consume the RX app-stream sidebands (Route 35-13 DPRA driverless-net fix): latch
     crc_error|app_tlast at rx_frame_done so the recovered-stream read bits stay connected
     to a real pin and can't be pruned. *)
  ...
  ; (* RX: saw a UDP payload SOF *)
    led0_b = gnd
  ; led1_r = wire_tx_busy
  ; (* TX: MAC frame in flight (btn[3]) *)
    led1_g = phy_ready
  ; (* PHY out of hard reset *)
    led1_b = wire_udp_busy
  ; (* TX: Udp_tx emitting a datagram *)
    led2_r = udp_inst.crc_error
  ; (* RX: held bad-FCS verdict (padding-safe) *)
    led2_g = udp_inst.checksum_ok
  ; (* RX: IPv4 header checksum verified *)
    led2_b = in_payload_tx
  ; (* RX: MAC actively receiving *)
    led3_r = udp_inst.crc_error
  ; (* RX: mirror — eye-catching "bad frame"    *)
    led3_g = gnd
  ; led3_b = udp_inst.rx_udp_busy
  ; (* RX: Udp_rx parser mid-datagram *)
    uart_rxd_out = vdd
  ; eth_mdc = gnd
