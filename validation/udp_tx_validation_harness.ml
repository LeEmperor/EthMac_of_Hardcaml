...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  (* A single btn[3] press emits exactly one UDP datagram of [app_payload_len] application
     bytes (incrementing 0x01, 0x02, …). Udp_tx wraps them in the IPv4 + UDP headers; the
     MAC prepends Ethernet framing and appends the FCS (zero-padding the Ethernet payload
     to 46 bytes if the datagram is shorter).

     app_payload_len = 18 makes the Ethernet payload exactly 46 bytes (20 IPv4 + 8 UDP +
     18 app), i.e. the minimum frame with NO MAC padding — the simplest case for first
     bring-up. Drop it below 18 to exercise the MAC zero-pad path (the host validator keys
     off the IPv4 total-length field, so it tolerates the trailing pad either way). *)
  ...
  in
  (* ── Consume the AXI-S sideband (tlast/tuser); anti-prunes the FIFO read bits exactly
     as in the MAC harness (Route 35-13 DPRA driverless-net fix). *)
  let rx_axis_crc_err =
    Signal.reg_fb
      spec_tx
      ~width:1
  ...
  ; led0_b = gnd
  ; led1_r = wire_tx_busy
  ; (* MAC TX frame in flight (btn[3]-triggered) *)
    led1_g = phy_ready
  ; (* PHY out of hard reset *)
    led1_b = wire_udp_busy
  ; (* Udp_tx emitting a datagram *)
