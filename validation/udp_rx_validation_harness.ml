...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  (* ── RX drain: pop one recovered UDP byte per second (mirrors the MAC harness). Gating
     app_tready with this pulse backpressures the whole IPv4/UDP chain, so the datagram
     buffers in the MAC's async RX FIFO and its application bytes are released
     one-per-second — eye-visible on led[3:0]. *)
  let rx_drain = Board_scaffolding.rx_drain ~scope ~clock:i.I.eth_tx_clk ~reset:tx_rst in
  (* ── The UDP-over-MAC *receive* stack ─────────────────────────────────────── *)
  (* RX-only top: the MAC TX sink is tied off inside Udp_rx_mac_top. app_tready is the
  ...
  in
  (* ── Each drained recovered UDP app byte → led[3:0] ─────────────────────────── Latch
     on the drain handshake (pulse & tvalid), exactly as the MAC harness latches its FIFO
     byte — so an alternating 0xAA/0x55 payload shows led[3:0] stepping 0xA <-> 0x5. *)
  let last_byte =
    Signal.reg spec_tx ~enable:(rx_drain.pulse &: udp_inst.app_tvalid) udp_inst.app_tdata
  in
  (* ── "Saw a valid datagram" — held once any UDP payload SOF is observed ────── *)
  let saw_valid_datagram =
    Signal.reg_fb spec_tx ~width:1 ~enable:vdd ~f:(fun q -> q |: udp_inst.app_start)
    -- "saw_valid_datagram"
  in
  (* ── Consume the app-stream sidebands (Route 35-13 DPRA driverless-net fix) ── Latch
     the app-visible bad-frame verdict at rx_frame_done so it survives, and fold app_tlast
     in too so synthesis can't prune the recovered-stream bits. *)
  let bad_frame_latched =
    Signal.reg_fb spec_tx ~width:1 ~enable:udp_inst.rx_frame_done ~f:(fun _q ->
      udp_inst.crc_error |: udp_inst.app_tlast)
    -- "bad_frame_latched"
  in
  ...
  ignore (bad_frame_latched : Signal.t);
  { O.led = Signal.select last_byte ~high:3 ~low:0
  ; led0_r = heartbeat_toggle
  ; led0_g = saw_valid_datagram
  ; led0_b = gnd
  ; led1_r = gnd
  ; (* no TX activity on an RX-only board *)
    led1_g = phy_ready
  ; (* PHY out of hard reset *)
    led1_b = udp_inst.udp_busy
  ; (* RX parser mid-datagram *)
    led2_r = udp_inst.crc_error
  ; (* held bad-FCS verdict (padding-safe) *)
    led2_g = udp_inst.checksum_ok
  ; (* IPv4 header checksum verified *)
    led2_b = in_payload_tx
  ; (* MAC RX actively receiving *)
    led3_r = udp_inst.crc_error
  ; (* mirror: eye-catching "bad frame"     *)
    led3_g = gnd
  ; led3_b = gnd
  ; uart_rxd_out = vdd
  ; eth_mdc = gnd
