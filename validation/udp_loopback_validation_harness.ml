...
let create (scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  (* No app_tready input: the bridge FSM pulls the recovered stream and drives it back
     into the TX interface internally. ~rx_fifo_for_sim:false selects the real async RX
     FIFO for the board. *)
  let udp_inst =
    Udp_loopback_mac_top.hierarchical
  ...
  let last_byte = Signal.reg spec_tx ~enable:udp_inst.app_tvalid udp_inst.app_tdata in
  ...
  (* ── Consume the app-stream sidebands (Route 35-13 DPRA driverless-net fix) ── Latch
     the app-visible bad-frame verdict at rx_frame_done and fold app_tlast in so synthesis
     can't prune the recovered-stream bits. *)
  ...
  let regs_inst =
    Mac_validation_harness_regs.hierarchical
      ...
      ; (* the echo drives the MII TX pins *)
        rx_dv = i.I.eth_rx_dv
      }
  in
  ...
  ; led0_b = gnd
  ; led1_r = udp_inst.tx_busy
  ; (* echo frame in flight on the MII pins *)
    led1_g = phy_ready
  ; (* PHY out of hard reset *)
    led1_b = udp_inst.bridge_active
  ; (* RX->TX bridge mid-echo *)
